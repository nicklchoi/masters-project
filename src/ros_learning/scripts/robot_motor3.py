...
def motor1_angle_callback(motor_msg):
    ...
    kit.servo[3].angle = motor_msg.motor3
    #kit.servo[1].angle = msg.motor2

    print("Moving servo 3 to " + str(motor_msg.motor3) + "degrees.")
    current_angle = 180
    kit.servo[3].angle = current_angle
    sleep(2)
    while current_angle > motor_msg.motor3:
        current_angle = current_angle - 1
        if fsr_val > 1000:
            print("Servo 3 pressure exceeded 20,000! Stopping Motor.")
            break
        kit.servo[3].angle = current_angle
        #print(current_angle)
        #print(fsr_val)
...
def listener():
    rospy.init_node('motor3_angle_sub', anonymous=True)
