...
def motor1_angle_callback(motor_msg):
    ...
    kit.servo[1].angle = motor_msg.motor1
    #kit.servo[1].angle = msg.motor2

    print("Moving servo 1 to " + str(motor_msg.motor1) + "degrees.")
    current_angle = 180
    kit.servo[1].angle = current_angle
    sleep(2)
    while current_angle > motor_msg.motor1:
        current_angle = current_angle - 1
        if fsr_val > 1000:
            print("Servo 1 pressure exceeded 20,000! Stopping Motor.")
            break
        kit.servo[1].angle = current_angle
        #print(current_angle)
        #print(fsr_val)
